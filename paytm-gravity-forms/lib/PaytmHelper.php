...
class PaytmHelper{
	...
	*/
	public static function getPaytmOrderId($order_id){
		if($order_id && PaytmConstantsGF::APPEND_TIMESTAMP){
			return PaytmConstantsGF::ORDER_PREFIX.$order_id . '_' . date("YmdHis");
		}else{
			return PaytmConstantsGF::ORDER_PREFIX.$order_id;
		}
	}
	/**
	...
	*/
	public static function getOrderId($order_id){		
		if(($pos = strrpos($order_id, '_')) !== false && PaytmConstantsGF::APPEND_TIMESTAMP) {
			$order_id = substr($order_id, 0, $pos);
		}
		if (substr($order_id, 0, strlen(PaytmConstantsGF::ORDER_PREFIX)) == PaytmConstantsGF::ORDER_PREFIX) {
			$order_id = substr($order_id, strlen(PaytmConstantsGF::ORDER_PREFIX));
		} 
		return $order_id;
	}
	...
	public static function getPaytmURL($url = false, $isProduction = 0){
		if(!$url) return false; 
		if($isProduction == 1){
			return PaytmConstantsGF::PRODUCTION_HOST . $url;
		}else{
			return PaytmConstantsGF::STAGING_HOST . $url;			
		}
	}
	/**
	* exclude timestap with order id
	*/
	public static function getTransactionStatusURL($isProduction = 0){		
		if($isProduction == 1){
			return PaytmConstantsGF::TRANSACTION_STATUS_URL_PRODUCTION;
		}else{
			return PaytmConstantsGF::TRANSACTION_STATUS_URL_STAGING;			
		}
	}
	/**
